...
function extractNameFromTemplate(value) {
  return value.match(/^Hello,\s([A-z\s]+)!$/)[1];
}
...
function getRectangleString(width, height) {
  ...
  for (let i = 0; i < height; i += 1) {
    for (let j = 0; j < width; j += 1) {
      if (i === 0) {
        if (j === 0) res.push('┌');
        else if (j === width - 1) res.push('┐');
        else res.push('─');
      }
      if (i < height - 1 && i > 0) {
        if (j === 0 || j === width - 1) res.push('│');
        else res.push(' ');
      }
      if (i === height - 1) {
        if (j === 0) res.push('└');
...
function encodeToRot13(str) {
  ...
  return str.replace(/([A-z]{1})/g, (match) => rot13[abc.indexOf(match)]);
}
