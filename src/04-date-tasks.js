...
function timeSpanToString(startDate, endDate) {
  function timeFormat(str) {
    ...
    let ms = str.substring(str.indexOf('.') + 1);
    ms = `00${ms}`.slice(-3);
    time = time.split(':').map((el) => `0${el}`.slice(-2)).join(':');
    return `${time}.${ms}`;
  }

  let milliseconds = endDate - startDate;
  let seconds = parseInt(milliseconds / 1000, 10);
...
function angleBetweenClockHands(date) {
  ...
  let degrees = Math.abs(60 * hours - 11 * minutes);
  degrees = (degrees > 360) ? degrees - 360 : degrees;
  return (0.5 * degrees * Math.PI) / 180;
}
