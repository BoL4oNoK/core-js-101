...
function findFirstSingleChar(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (str.indexOf(str[i], i + 1) < 0 && str.indexOf(str[i]) === i) res.push(str[i]);
  }
...
function isCreditCardNumber(ccn) {
  let sum = 0;
  const arrCCN = ccn.toString().split('');
  const isEven = (arrCCN.length % 2 === 0);
  arrCCN.forEach((el, ind) => {
    let num = Number(el);
    if (isEven) {
      if (ind % 2 === 0) {
        num *= 2;
...
function isBracketsBalanced(str) {
  ...
  const pair = /(\[\])|(\(\))|(\{\})|(<>)/i;
  const reg = new RegExp(pair, 'g');
  let subStr;
  let strBase = str;
  while (subStr !== strBase) {
    subStr = strBase;
    strBase = strBase.replace(reg, '');
  }
